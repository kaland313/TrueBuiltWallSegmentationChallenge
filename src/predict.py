...
def process_architectural_drawing(
    input_path, output_dir, model_ckpt="model.ckpt", gpu_id=0
):
    """
    ...
    try:
        ...
        cv2.imwrite(
            str(output_dir / f"{base_name}_segmented.png"), predicted_mask * 100
        )
        ...
        cv2.imwrite(
            str(output_dir / f"{base_name}_segmented_overlay.png"), overlayed_initial
        )
        overlayed_cleaned = overlay_mask(image, cleaned_mask)
        cv2.imwrite(
            str(output_dir / f"{base_name}_cleaned_overlay.png"), overlayed_cleaned
        )
...
def main():
    ...
    parser.add_argument(
        "input_path", help="Input PNG file or folder containing PNG files"
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    for image_file in png_files:
        if process_architectural_drawing(
            image_file, output_folder, model_ckpt=args.model_ckpt, gpu_id=args.gpu_id
        ):
            successful += 1
        print()
